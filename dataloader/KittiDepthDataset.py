...
class KittiDepthDataset(Dataset):
    def __init__(self, data_path, gt_path,
                 # gt_bincls_path, gt_resreg_path,
                 setname='train', transform=None, norm_factor=256, invert_depth=False,
                 rgb_dir=None, rgb2gray=False, fill_depth=False, flip=False, blind=False):
        self.data_path = data_path
        self.gt_path = gt_path
        # self.gt_bincls_path = gt_bincls_path
        # self.gt_resreg_path = gt_resreg_path
    ...
    def __getitem__(self, item):
        ...
        if self.setname == 'train' or self.setname == 'val':
            ...
            # gt_bincls_path = self.gt_bincls[item].split(self.setname)[1]
            # gt_resreg_path = self.gt_resreg[item].split(self.setname)[1]
            gt_path = self.gt[item].split(self.setname)[1]

            # Check folder name
            # assert (data_path[0:25] == gt_path[0:25] == gt_bincls_path[0:25] == gt_resreg_path[0:25])

            data_path = data_path.split('image')[1]
            # gt_bincls_path = gt_bincls_path.split('image')[1]
            # gt_resreg_path = gt_resreg_path.split('image')[1]
            gt_path = gt_path.split('image')[1]

            # Check filename
            assert (data_path[:-4] == gt_path[:-4])
        ...
        # gt_bincls = np.load(str(self.gt_bincls[item]))
        # gt_resreg = np.load(str(self.gt_resreg[item]))
        ...
        if self.transform is not None:
            ...
            # DO NOT SUPPORT 'center crop'
            # assert False, "DO NOT SUPPORT 'center crop'"
            # gt_bincls = self.transform(gt_bincls)
            # gt_resreg = self.transform(gt_resreg)
        ...
        if self.transform is None and self.setname == 'train':
            ...
            # gt_bincls = gt_bincls[crop_lt_v: crop_lt_v + 352, crop_lt_u: crop_lt_u+1216]
            # gt_resreg = gt_resreg[crop_lt_v: crop_lt_v + 352, crop_lt_u: crop_lt_u+1216]
        ...
        if self.flip and random.randint(0, 1) and self.setname == 'train':
            ...
            # gt_bincls = np.fliplr(gt_bincls).copy()
            # gt_resreg = np.fliplr(gt_resreg).copy()
        ...
        # gt_resreg = np.expand_dims(gt_resreg, 0)
        ...
        # gt_bincls = torch.tensor(gt_bincls, dtype=torch.long)
        # gt_resreg = torch.tensor(gt_resreg, dtype=torch.float)
        ...
        if self.invert_depth:
            assert False, "DO NOT SUPPORT 'invert_depth'"
            data[data == 0] = -1
            data = 1 / data
            data[data == -1] = 0
        ...
        # return data, C, item, rgb, gt_bincls, gt_resreg, gt
        return data, C, item, rgb, gt
